...
function update_stats() {
    ...
    "| \(.key) | \(((.value.plugins_with_errors + .value.themes_with_errors) / (.value.plugins_tested + .value.themes_tested) * 100 | floor * 0.001 ))% | \(.value.plugins_tested) | \(.value.themes_tested) | \(.value.plugins_with_errors) | \(.value.themes_with_errors) |"' \
